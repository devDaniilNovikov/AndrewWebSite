...
Concurrent requests that reuse one `requestId` are not serialized; the form never submits the same attempt in parallel. A response lost after Telegram accepted the message can lead to a second message on retry; the recipient recognizes it by the `requestId` line.
