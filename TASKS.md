...
| 4 | `fix-preserve-merged-branches` | 1 | Codex | `task-backend-skeleton` merged | `merged` | `fix-preserve-merged-branches` (retained) | Reconcile clean-worktree state live | [PR #19](https://github.com/devDaniilNovikov/AndrewWebSite/pull/19) | Squash merge `288fdd5b8fdba3cfbb772babed466c5c6bcd934a`; post-merge state must remain subject to live verification | Merged under explicit authorization |
| 5 | `task-agents-memory` | 1 | Codex | `fix-preserve-merged-branches` merged | `draft_pr` | `task-agents-memory` | `/Users/daniilnovikov/.codex/worktrees/agents-memory/AndrewWorkWebSite` | [PR #20](https://github.com/devDaniilNovikov/AndrewWebSite/pull/20) | Fresh GitHub PR checks will be required after this metadata commit is pushed; pre-push local documentation, link, scope, and credential checks plus independent SPEC PASS and QUALITY PASS are complete | User merge authorization only after fresh green gates |
| 6 | `task-ci-backend-gates` | 1 | Jules | `task-agents-memory` / PR #20 merged; separately owned `fix-backend-skeleton-review` Draft PR explicitly merged after non-automatic authorization; follow-on owner reconciled `TASKS.md`; separately authorized Jules dispatch | `blocked` | `task-ci-backend-gates` | Jules creates fresh after dispatch | Issue and PR pending | actionlint, Maven verify, dependency/security gates, CI, review | Not authorized; do not create a Jules Issue or dispatch before all prerequisites and separate user authorization |
| 7 | `task-backend-deploy-stub` | 1 | Codex | `task-ci-backend-gates` merged | `blocked` | pending | pending | pending | Maven, Docker build/smoke, secrets, CI, reviews | Not authorized |
| 8 | `task-backend-http-security` | 2 | Codex | previous task merged | `blocked` | pending | pending | pending | TDD, Maven verify, coverage, security, CI, reviews | Not authorized |
| 9 | `task-db-flyway-baseline` | 2 | Codex | previous task merged | `blocked` | pending | pending | pending | TDD, PostgreSQL 17 migrations/constraints, Maven verify, CI, reviews | Not authorized |
| 10 | `task-leads-api` | 2 | Codex | previous task merged | `blocked` | pending | pending | pending | TDD, API/idempotency/transaction/race tests, Maven verify, security, CI, reviews | Not authorized |
| 11 | `task-telegram-client` | 3 | Codex | previous task merged | `blocked` | pending | pending | pending | TDD, fake Telegram failures, Maven verify, secrets, CI, reviews | Not authorized |
| 12 | `task-telegram-worker` | 3 | Codex | previous task merged | `blocked` | pending | pending | pending | TDD, two-worker/lease/retry tests, Maven verify, telemetry, CI, reviews | Not authorized |
| 13 | `task-lead-retention` | 4 | Codex | previous task merged | `blocked` | pending | pending | pending | TDD, 29/30-day boundaries, replay after anonymization, Maven verify, CI, reviews | Not authorized |
| 14 | `task-backend-observability` | 4 | Codex | previous task merged | `blocked` | pending | pending | pending | TDD, degraded dependencies, PII-free telemetry, Maven verify, CI, reviews | Not authorized |
| 15 | `task-static-jar-integration` | 5 | Codex | `task-backend-observability` and Claude-owned frontend static-export scaffold merged | `blocked` | pending | pending | pending | frontend build/tests, Maven verify, container smoke, routing/cache/404/API/health checks, CI, reviews | Not authorized |

## Dispatch boundary for Jules

The skeleton and branch-retention prerequisites are satisfied, but no stacked work is allowed. Do not create the `task-ci-backend-gates` Issue or dispatch Jules until this complete sequence is verified live: (1) PR #20 / `task-agents-memory` merges; (2) the separately owned `fix-backend-skeleton-review` owner rebases and opens its own Draft fix PR only; (3) that fix PR receives explicit, non-automatic merge authorization and merges; (4) its owner reconciles `TASKS.md`; and (5) the user separately authorizes Jules dispatch. The memory flow neither starts nor merges the fix PR. Only then may the allowed repository owner create a new sanitized Issue and apply exactly one label, `jules-action`. Never apply `jules`, never combine both labels, and never label an untrusted source Issue.
